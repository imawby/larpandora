...
void IvysaurusTrainingFiles::analyze(const art::Event &evt)
{
    ...
    for (const art::Ptr<recob::PFParticle> &pfparticle : pfparticles)
    {
        ...
        if (TruthMatchUtils::Valid(g4id))
        {
            ...
            m_trueEndX = mcParticle->EndX(); 
            m_trueEndY = mcParticle->EndY();
            m_trueEndZ = mcParticle->EndZ();
        }
        else
        {
            continue;
             ...
             IvysaurusUtils::PandoraView::TPC_VIEW_V, IvysaurusUtils::PandoraView::TPC_VIEW_W})
        {
            ...
            //startGridValues = float[256](dimensions, std::vector<float>(dimensions, 0.f));
            //endGridValues = float[256](dimensions, std::vector<float>(dimensions, 0.f));            

            for (unsigned int driftIndex = 0; driftIndex < dimensions; ++driftIndex)
            {
                int index = (driftIndex * dimensions);
